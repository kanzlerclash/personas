...
N=$(node -e 'const fs=require("fs");const S="landwirt handwerkerin pendler rentnerpaar polizist studentin_queer alleinerziehende pflegekraft eingebuergerte mieterin industriearbeiter energie_strukturwandel junge_familie_bau solo_kreative landaerztin soldat".split(" ");const P="cdu spd gruene afd fdp linke bsw".split(" ");let n=0;for(const pa of P)for(const sl of S){try{const d=JSON.parse(fs.readFileSync(`ergebnisse/${sl}/sachsen-anhalt/${pa}/gemini-3.1-pro__2026-06-30__lauf1.json`));if(Array.isArray(d.besonders_gut)&&d.gesamt&&d.modell_slug)n++;}catch(e){}}console.log(n);')
