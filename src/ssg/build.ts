...
const vsScores = (a: number, b: number) => {
  ...
  return `<span class="splitscore">${isNaN(a) ? '<span class="meta">—</span>' : urteilPill(a)}<span class="vs">vs</span>${isNaN(b) ? '<span class="meta">—</span>' : urteilPill(b)}${isNaN(d) ? "" : `<span class="delta${d >= 2 ? " hoch" : ""}" title="Δ Modell-Urteil: Differenz der Urteile (0–4)">${d ? "Δ" + d : "="}</span>`}</span>`;
};
...
<div class="zahlkarte" id="delta"><h4>Δ Modell-Urteil</h4><div class="zk-skala">Konsens · nahezu einig · uneinig · stark uneinig · maximaler Gegensatz <span class="zk-form">(intern 0…4)</span></div><p>Differenz der Modell-Urteile <em>zweier</em> Modelle für denselben Fall (auf den A-vs-B-Seiten) — extern als Wort. Zeigt, wie stark das Urteil vom gewählten Modell abhängt.</p><p class="bsp"><strong>Beispiel:</strong> ${e(_bName)} × ${e(_bPartei)}: ${e(kurz(_bLo.slug))} „${e(_bLab(_bLo))}" vs ${e(kurz(_bHi.slug))} „${e(_bLab(_bHi))}" → ${e(spanneLabel(_bD))} (Δ ${_bD}).</p></div>
...
<div class="detail-kopf">${avatarImg(p, "avatar gross")}<div>${kopfVs(` — ${e(p.name)} × ${e(parteiName(pa))}`)}${fiktiv(true)}${isNaN(dScore) ? "" : `<p class="einzeiler">Divergenz der Modelle: <strong>${e(spanneLabel(dScore))}</strong>${hilfe("delta", "Δ Modell-Urteil: Differenz der beiden Urteile, 0–4")} <span class="meta">(Δ ${dScore})</span></p>`}</div></div>
<a class="navbtn profil-link" href="${u(`persona/${p.slug}/`)}">📋 Vollständiges Profil von ${e(p.name)} →</a>
